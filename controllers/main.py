...
class BMLController(http.Controller):
    _return_url = '/payment/bml/return'
    _version = '1.0.0'
    ...
    def bml_return_from_checkout(self, reference, **data):
        """ Process the notification data sent by BML after redirection from checkout.

        :param str reference: The transaction reference embedded in the return URL.
        :param dict data: The notification data.
        """
        
        _logger.info("Handling redirection from BML with data:\n%s", pprint.pformat(data))
        if all(f'{key}' in data for key in ('OrderID', 'AcquirerId', 'Signature')):
            # Check the integrity of the notification.
            tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data(
                'bml', {'OrderID': reference}
            )              
            self._verify_notification_signature(data, data.get('Signature'), tx_sudo)

            # Handle the notification data.
            tx_sudo._handle_notification_data('bml', data)
    ...
    def _verify_notification_signature(
        ...
        expected_signature = tx_sudo.provider_id._bml_calculate_signature(
